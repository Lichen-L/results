...
#def getMevForNested(V,availability,nests) :
    ...
    #for m in nests:
        ...
        #counter=0
        #for i in m[1]:
            #sum += Elem({0:0.0,1: y[i] ** m[0]},availability[i]!=0)
            #counter=counter+1
            ##print(i)
            #if (counter%500==0):    
                #d[j]= sum
...
beta_cons_motor = Beta('constant for motor',0,-20,20,0)
...
#MU1 = Beta('MU for all destinations by pt', 1.5,1,100,1)
#MU2 = Beta('MU for all destinations by cars',1.5,1,100,1)
#MU3 = Beta('MU for all others',1,1,100,1)
...
for j in range(1,1093):
	...
	exec("V_%s = beta_cons_bus+cost_over_income_bus_%s * (1-missing_income) * beta_cost_bus_mrt_1 + cost_bus_%s*missing_income*beta_cost_bus_mrt_2 + tt_bus_%s * beta_tt_bus_mrt+beta_central_bus_mrt * central_dummy_%s + beta_log * log(employment_%s+exp(beta_area)*area_%s+exp(beta_population)*population_%s) + (walk_distance_first_%s+walk_distance_second_%s)*beta_distance_bus_mrt + beta_female_bus * Female_dummy" % (V_counter,j,j,j,j,j,j,j,j,j)) 

#mrt
for j in range(1,1093):
	V_counter=V_counter+1
	exec("V_%s = beta_cons_mrt+cost_over_income_mrt_%s * (1-missing_income) * beta_cost_bus_mrt_1 + cost_mrt_%s*missing_income*beta_cost_bus_mrt_2 + tt_mrt_%s * beta_tt_bus_mrt+beta_central_bus_mrt * central_dummy_%s + beta_log * log(employment_%s+exp(beta_area)*area_%s+exp(beta_population)*population_%s) + (walk_distance_first_%s+walk_distance_second_%s)*beta_distance_bus_mrt + beta_female_mrt * Female_dummy" % (V_counter,j,j,j,j,j,j,j,j,j)) 

#private_bus
for j in range(1,1093):
	V_counter=V_counter+1
	exec("V_%s = beta_cons_private_bus+cost_over_income_private_bus_%s * (1-missing_income) * beta_cost_private_bus_1 + cost_private_bus_%s*missing_income*beta_cost_private_bus_2 + tt_private_bus_%s * beta_tt_bus_mrt+beta_central_private_bus * central_dummy_%s + beta_log * log(employment_%s+exp(beta_area)*area_%s+exp(beta_population)*population_%s+1) + (walk_distance_first_%s+walk_distance_second_%s)*beta_distance_private_bus + beta_female_private_bus * Female_dummy" % (V_counter,j,j,j,j,j,j,j,j,j)) 

#drive1
for j in range(1,1093):
	V_counter=V_counter+1
	exec("V_%s = beta_cons_drive1+cost_over_income_drive1_%s * (1-missing_income) * beta_cost_drive1_1 + cost_drive1_%s*missing_income*beta_cost_drive1_2 + tt_drive1_%s * beta_tt_drive1+beta_central_drive1 * central_dummy_%s + beta_log * log(employment_%s+exp(beta_area)*area_%s+exp(beta_population)*population_%s+1) + (walk_distance_first_%s+walk_distance_second_%s)*beta_distance_drive1 + beta_zero_drive1 * zero_car + beta_oneplus_drive1 * one_plus_car + beta_twoplus_drive1 * two_plus_car + beta_threeplus_drive1 * three_plus_car +  beta_female_drive1 * Female_dummy" % (V_counter,j,j,j,j,j,j,j,j,j))

#share2
for j in range(1,1093):
	V_counter=V_counter+1
	exec("V_%s = beta_cons_share2+cost_over_income_share2_%s * (1-missing_income) * beta_cost_share2_1 + cost_share2_%s*missing_income*beta_cost_share2_2 + tt_share2_%s * beta_tt_drive1+beta_central_share2 * central_dummy_%s + beta_log * log(employment_%s+exp(beta_area)*area_%s+exp(beta_population)*population_%s+1) + (walk_distance_first_%s+walk_distance_second_%s)*beta_distance_share2 + beta_zero_share2 * zero_car + beta_oneplus_share2 * one_plus_car + beta_twoplus_share2 * two_plus_car + beta_threeplus_share2 * three_plus_car + beta_female_share2 * Female_dummy" % (V_counter,j,j,j,j,j,j,j,j,j))

#share3
for j in range(1,1093):
	V_counter=V_counter+1
	exec("V_%s = beta_cons_share3+cost_over_income_share3_%s * (1-missing_income) * beta_cost_share3_1 + cost_share3_%s*missing_income*beta_cost_share3_2 + tt_share3_%s * beta_tt_drive1+beta_central_share3 * central_dummy_%s + beta_log * log(employment_%s+exp(beta_area)*area_%s+exp(beta_population)*population_%s+1) + (walk_distance_first_%s+walk_distance_second_%s)*beta_distance_share3 + beta_zero_share3 * zero_car + beta_oneplus_share3 * one_plus_car + beta_twoplus_share3 * two_plus_car + beta_threeplus_share3 * three_plus_car + beta_female_share3 * Female_dummy" % (V_counter,j,j,j,j,j,j,j,j,j))

#motor
for j in range(1,1093):
	V_counter=V_counter+1
	exec("V_%s = beta_cons_motor+cost_over_income_motor_%s * (1-missing_income) * beta_cost_motor_1 + cost_motor_%s*missing_income*beta_cost_motor_2 + tt_motor_%s * beta_tt_drive1+beta_central_motor * central_dummy_%s + beta_log * log(employment_%s+exp(beta_area)*area_%s+exp(beta_population)*population_%s+1) + (walk_distance_first_%s+walk_distance_second_%s)*beta_distance_motor + beta_zero_motor * zero_motor + beta_oneplus_motor * one_plus_motor + beta_twoplus_motor * two_plus_motor + beta_threeplus_motor * three_plus_motor + beta_female_motor * Female_dummy" % (V_counter,j,j,j,j,j,j,j,j,j))


#walk
for j in range(1,1093):
	V_counter=V_counter+1
	exec("V_%s = beta_cons_walk+tt_walk_%s*beta_tt_walk +beta_central_walk * central_dummy_%s+ beta_log * log(employment_%s+exp(beta_area)*area_%s+exp(beta_population)*population_%s+1) + (walk_distance_first_%s+walk_distance_second_%s)*beta_distance_walk + beta_female_walk * Female_dummy" % (V_counter,j,j,j,j,j,j,j))

#taxi	
for j in range(1,1093):
	V_counter=V_counter+1
	exec("V_%s = beta_cons_taxi+cost_over_income_taxi_%s *(1-missing_income)* beta_cost_taxi_1 + cost_taxi_%s*missing_income*beta_cost_taxi_2+ tt_taxi_%s * beta_tt_taxi+beta_central_taxi * central_dummy_%s + beta_log * log(employment_%s+exp(beta_area)*area_%s+exp(beta_population)*population_%s+1) + (walk_distance_first_%s+walk_distance_second_%s)*beta_distance_taxi + beta_female_taxi * Female_dummy" % (V_counter,j,j,j,j,j,j,j,j,j)) 
...
#prob = cnl_avail(V,av,nests,combined_choice_new)
...
exclude = ((combined_choice==0)+(combined_choice_new==0)+(combined_violation==1)+(IncomeIndex==12)) > 0

BIOGEME_OBJECT.EXCLUDE = exclude

#nullLoglikelihood(av,'obsIter')
#choiceSet = range(1,(9*1092+1))
#cteLoglikelihood(choiceSet,combined_choice_new,'obsIter')
